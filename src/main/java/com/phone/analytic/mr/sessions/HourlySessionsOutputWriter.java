 ...
 * @since 1.0
 */
public class HourlySessionsOutputWriter implements IOutputWriter {

    private static Logger logger = Logger.getLogger(HourlySessionsOutputWriter.class);
    ...
    public void setParams(Configuration conf, StatsBaseDimension key,
        ...
        try {
            ...
            int kpiId = iDimension.getDimensionIdByObject(new KpiDimension(v.getKpi().kpiName));
            //从value中获取会话个数和总时长
            int i = 1;
            //匹配kpi分别赋值
            ps.setInt(i++,dateId);
